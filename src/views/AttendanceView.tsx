      ...
      {renewalMember && (
        <div className="modal-overlay" onClick={() => { setRenewalMember(null); setRenewalType(null); }}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '500px' }}>
            ...
            {renewalType === 'plan' && (
              <div style={{ background: 'color-mix(in srgb, var(--brand-accent) $(("{0:N0}" -f ([double]0.04 * 100)) | %{ if (import { useState, useEffect, useCallback } from 'react';
import * as api from '../api';
import type { CheckIn, Member } from '../types';
...
export default function AttendanceView({ showNotification }: { showNotification: (message: string, type?: 'success' | 'error') => void }) {
 ...
 -eq "0") { "0" } else { import { useState, useEffect, useCallback } from 'react';
import * as api from '../api';
import type { CheckIn, Member } from '../types';
...
export default function AttendanceView({ showNotification }: { showNotification: (message: string, type?: 'success' | 'error') => void }) {
 ...
 } })%, transparent)', border: '1px solid color-mix(in srgb, var(--brand-accent) $(("{0:N0}" -f ([double]0.2 * 100)) | %{ if (import { useState, useEffect, useCallback } from 'react';
import * as api from '../api';
import type { CheckIn, Member } from '../types';
...
export default function AttendanceView({ showNotification }: { showNotification: (message: string, type?: 'success' | 'error') => void }) {
 ...
 } })%, transparent)', borderRadius: '12px', padding: '1.25rem', marginBottom: '1rem' }}>
                <h5 style={{ margin: '0 0 1rem 0', color: 'var(--brand-accent)', fontWeight: 600, fontSize: '0.95rem' }}>Plan Renewal</h5>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '0.8rem', marginBottom: '1rem' }}>
                  <div className="form-group" style={{ margin: 0 }}>
                    <label>New Plan</label>
