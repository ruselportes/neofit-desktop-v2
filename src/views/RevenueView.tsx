  ...
  const chartConfig = useMemo(() => {
    ...
    return {
      ...
      datasets: [{
        ...
        fill: true,
        tension: 0.4,
        borderColor: getComputedStyle(document.documentElement).getPropertyValue('--brand-accent').trim() || '#ea580c',
        backgroundColor: (ctx: { chart: { ctx: CanvasRenderingContext2D } }) => {
          const accent = getComputedStyle(document.documentElement).getPropertyValue('--brand-accent').trim() || '#ea580c';
        ...
        <div className="rev-dashboard">
          ...
          <div className="rev-kpi-row">
            <div className="rev-kpi-card">
              <div className="rev-kpi-icon" style={{ background: 'color-mix(in srgb, var(--brand-accent) 12%, transparent)', color: 'var(--brand-accent)' }}>
        ...
        <div className="modal-overlay" onClick={() => setBreakdownModal(null)}>
          <div className="modal-content" style={{ maxWidth: '520px' }} onClick={e => e.stopPropagation()}>
            ...
            <h3 style={{ marginBottom: '1.25rem' }}>
              {breakdownModal === 'checkin' ? 'Check-in' : 'Payment'} Revenue â€” {monthNames[month - 1]} {year}
            </h3>
              ...
              {(() => {
                ...
                return rows.length > 0 ? rows.map(d => (
                  ...
                  >
                    ...
                    <span style={{ width: '100px', textAlign: 'right', fontSize: '0.85rem', fontWeight: 600, color: type === 'checkin' ? 'var(--brand-accent)' : 'var(--success)' }}>
            ...
            <div style={{ marginTop: '1rem', padding: '0.75rem', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: '1.1rem', fontWeight: 700, color: breakdownModal === 'checkin' ? 'var(--brand-accent)' : 'var(--success)' }}>
      ...
      {detailModal && (
        <div className="modal-overlay" onClick={() => setDetailModal(null)}>
          <div className="modal-content" style={{ maxWidth: '560px' }} onClick={e => e.stopPropagation()}>
            ...
            <h3 style={{ marginBottom: '1.25rem' }}>
              {detailModal.type === 'checkin' ? 'Check-in' : 'Payment'} Details â€” {detailModal.date}
            </h3>
            {detailLoading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', maxHeight: '360px', overflowY: 'auto' }}>
                  ...
                  {detailRecords.map((r, i) => (
                    <div key={r.id ?? i} style={{ display: 'flex', alignItems: 'center', padding: '0.6rem 0.75rem', borderRadius: '6px' }}>
                      ...
                      <span style={{ width: '100px', textAlign: 'right', fontSize: '0.85rem', fontWeight: 600, color: detailModal.type === 'checkin' ? 'var(--brand-accent)' : 'var(--success)' }}>
      ...
      {planModal && (
        <div className="modal-overlay" onClick={() => setPlanModal(null)}>
          <div className="modal-content" style={{ maxWidth: '700px' }} onClick={e => e.stopPropagation()}>
            ...
            <h3 style={{ marginBottom: '1.25rem' }}>{planModal} â€” Members</h3>
            {planLoading ? (
